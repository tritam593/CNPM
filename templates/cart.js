const product = [
{image: '../data/images/balo/balo1/minibalo2.png',
title: 'Mini Album',
price: 120},
{image: '../data/images/chair/chair2/i1.jpg',
title: 'Chair',
price: 80},
{image: '../data/images/keyboard/keyboard1/keyboard_1_1.jpg',
title: 'keyboard',
price: 100},
{image: '../data/images/handbag/handbag2/tui_xach_xanh.png',
title: 'Handbag',
price: 2000},
]

const category = [...new Set(product.map((item)=> {return item}))]

function delElement(a){
    category.splice(a,1);
    displaycart();
}
function displaycart(){
    let j = 0, total = 0; 
    document.getElementById("CountA").innerHTML= category.length+ " Items";
    document.getElementById("CountB").innerHTML = category.length + " Items";

    if (category.length==0){

        document.getElementById("root").innerHTML="Your cart is empty";
        document.getElementById("Subtotal").innerHTML = "$ 00.00";
        document.getElementById("TotalPrice").innerHTML = "$ 00.00";

    } else {
        document.getElementById("root").innerHTML = category.map((items)=>{
            let {image, title,price} = items;
            total = total + price;
            document.getElementById("Subtotal").innerHTML = "$" + total +".00";
            if (total > 3000){
                document.getElementById("Ship").innerHTML = "Freeship";
                document.getElementById("TotalPrice").innerHTML = "$" + total + ".00";
            } else {
                document.getElementById("Ship").innerHTML = '$' + (25) +".00";
                document.getElementById("TotalPrice").innerHTML = "$" + (total + 25) + ".00";
            }
            return(
                `<tr>
                    <td>
                      <i class="fa-regular fa-circle-xmark" onclick='delElement("+ (j++) + ")'></i> 
                    </td>
                    <td> 
                        <div class = 'img-box'> <img class="img" src=${image}> </div> 
                    </td> 
                    <td> <p style='font-size:15px;'> ${title}</p> </td>
                    <td> <h2 style = 'font-size:15px; color:red;'>$ ${price}.00 </h2> </td>
                </tr>`

            );
        }).join('');
    }
}
displaycart();